...
// Custom layers store mock — state is replaced in beforeEach so each test
// starts clean, and individual tests can provide pre-populated layers.
...
// Stub the CustomLayerEditor so the tiles tests only cover TilesControl logic.
vi.mock('./custom-layer-editor', () => ({
  CustomLayerEditor: () => <div data-testid="custom-layer-editor-stub" />,
}));
...
describe('TilesControl', () => {
  ...
  describe('custom layers section', () => {
    ...
    it('should render a visibility switch for each custom layer', () => {
      mockCustomLayersState.layers = [
    ...
    it('should render a remove button for each custom layer', () => {
      ...
      // The remove button uses a Trash2 icon; we look for its accessible role.
      const buttons = screen.getAllByRole('button');
      expect(buttons.some((b) => b.className.includes('destructive'))).toBe(
        true
      );
    });
  ...
  describe('custom layer visibility toggle', () => {
    it('should call map.setLayoutProperty when toggling a custom layer off', async () => {
      ...
      // Layer must be on the map for setLayoutProperty to be called.
      mockMap.addLayer({ id: 'my-toggle-layer' });

      render(<TilesControl />);

      const switches = screen.getAllByRole('switch');
      const customLayerSwitch = switches[switches.length - 1]!;
  ...
  describe('styledata re-applies custom layers', () => {
    // In React, children's effects fire before parent effects.
    // ValhallaLayersToggle (child) registers the first styledata handler;
    // TilesControl (parent) registers the last one.
    ...
    it('should not re-add a custom layer that is already present on the map', async () => {
      ...
      // addLayer should not have been called again for the already-present layer
